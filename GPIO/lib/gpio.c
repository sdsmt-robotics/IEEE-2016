...
// version 1.3    January 31, 2016 - Python wrapper
...
//see http://dan.iel.fm/posts/python-c-extensions/ for help
...
#define EXYNOS_5410 0x13400000
#define EXYNOS EXYNOS_5410
// subtract 4 from DATA register base to get CON reg
#define GPIO_GPCONREG 4
// add 4 to DATA register base to get UPD register
#define GPIO_UPDOWN	4
// add 8 to DATA register base to get drive str control reg
#define GPIO_DRIVESTR 8
...
static PyObject* py_setup_gpiopin(PyObject* self, PyObject* args) {
	...
	if (!PyArg_ParseTuple(args, "iiii", &channel, &bit, &pullval, &value))
        return NULL;
...
static PyObject* py_gpio_read(PyObject* self, PyObject* args) {
	...
	if (!PyArg_ParseTuple(args, "ii", &channel, &bit))
        return NULL;
...
// Note: expect it to take around 800 clocks after setting a output bit for it
// to show up when you read the bits from that GPIO register again
static PyObject* py_gpio_write(PyObject* self, PyObject* args) {
	int output, channel, bit;
	int tmp, tmp1;
	unsigned char val;

	if (!PyArg_ParseTuple(args, "iii", &channel, &bit, &output))
        return NULL;
...
static PyObject* py_gpio_toggle(PyObject* self, PyObject* args) {
	...
	if (!PyArg_ParseTuple(args, "iii", &channel, &bit, &count))
        return NULL;
...
// METH_VARARGS defined in Python.h?
static PyMethodDef gpioMethods[] = {
    { "sys_init", py_setup_gpio, METH_VARARGS },
    { "pinMode", py_setup_gpiopin,METH_VARARGS },
	{ "digitalWrite", py_gpio_write, METH_VARARGS },
	{ "digitalRead", py_gpio_read, METH_VARARGS },
	{ "pinToggle", py_gpio_toggle, METH_VARARGS },
	{ "sys_clean", py_gpio_shutdown, METH_VARARGS },
    { NULL, NULL, 0, NULL } /* Sentinel */
};

// named init{library name}()
void initgpio()
{
	(void) Py_InitModule("gpio", gpioMethods);
}
